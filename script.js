...
const debounceSearchApi = debounce((text) => {
  searchApi(text);
}, 500);
...
async function searchApi(text) {
  ...
  if (typed_text != "") {
    let res = await fetch(
      `https://www.omdbapi.com/?i=tt3896198&apikey=8456bf09&s=${typed_text}&page=1`
    );
      ...
      for (let i = 0; i < resjson.Search.length; i++) {
        ...
        if (favourites != null) {
          for (let j = 0; j < favourites.length; j++) {
            if (
              resjson.Search[i].Title == favourites[j].title &&
              resjson.Search[i].Year == favourites[j].year
            ) {
              temp = true;
            }
          }
...
function addToFavourite(e, element) {
  ...
  for (let i = 0; i < favourites.length; i++) {
    if (
      favourites[i].title == movie.title &&
      favourites[i].year == movie.year
    ) {
      present = true;
      break;
    }
